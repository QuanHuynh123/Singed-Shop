	...
	<div id="content_50">
		...
		<c:if test="${ productPaginate.size() > 0 }">
			<ul class="list_product">
				<c:forEach var="StyleQuanAo" items="${productPaginate }" varStatus="loop">
					<li class="product">
						<a href="<c:url value="/trang-chu/product/${StyleQuanAo.idProduct}/${StyleQuanAo.idStyle}"/>">
							<div class="img_product" style="background-image: url(<c:url value="/static/product/${StyleQuanAo.image}"/>)"> </div>
						</a>
					...
					<c:if test="${ (loop.index + 1) % 4 == 0 || (loop.index + 1) == productPaginate.size() }">
							</ul>
		...
		<div class="pagination">
			<c:forEach var="item " begin="${ 1 }" end="${ paginateInfo.totalPage}" varStatus="loop">
				<c:if test="${ (loop.index) == paginateInfo.currentPage }">
					<a href="<c:url value="/trang-chu/style/${id }/${loop.index }"/>" class="active">${loop.index}</a>
				</c:if>
				<c:if test="${ (loop.index) != paginateInfo.currentPage }">
					<a href="<c:url value="/trang-chu/style/${idPhongCach }/${loop.index }"/>">${loop.index}</a>
				</c:if>
			</c:forEach>
		</div>
